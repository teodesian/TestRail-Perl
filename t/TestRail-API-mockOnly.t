...
is( scalar(@$runs), 4,
    "getChildRuns with multi-configs in the same group returns correct # of runs"
);

my $summary = $tr->getPlanSummary( $plan->{'id'} );
is( $summary->{'plan'}, 1094, "Plan ID makes it through in summary method" );
is( $summary->{'totals'}->{'untested'},
    4, "Gets total number of tests correctly" );
is( $summary->{'percentages'}->{'untested'},
    '100.00%', "Gets total percentages correctly" );
...
$projResType =
  $tr->getTestResultFieldByName( 'step_results', $project->{'id'} );
is( $projResType->{'id'}, 6,
    "Can get result field by name, AND filter by project ID" );
